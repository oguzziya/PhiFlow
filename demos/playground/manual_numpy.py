...
for RUN_GPU in [False, True]:
    for RES in resolutions:
        ...
        for i in range(STEPS):
            ...
            if RUN_GPU:
                x_rho = device_put(x_rho)
                v_rho = device_put(v_rho)
            ...
            sample_start = time.time()
            x_rho = DENSITY.sample_at(onp.asarray(x_rho))
            sample_end = time.time()
            sample_counter += sample_end - sample_start
            ...
            for component in VELOCITY.unstack():
                ...
                advection_start = time.time()
                if RUN_GPU:
                    x_vel = device_put(x_vel)
                    v_vel = device_put(v_vel)
                ...
                sample_start = time.time()
                x_vel = component.sample_at(onp.asarray(x_vel))
                sample_end = time.time()
                sample_counter += sample_end - sample_start
            ...
            DENSITY = DENSITY.with_data(onp.asarray(x_rho))
            VELOCITY = VELOCITY.with_data([onp.asarray(component) for component in x_vel_list])
