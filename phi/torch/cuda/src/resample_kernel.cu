...
void Resample2DCudaKernel (
	...
	for (unsigned int i = batch * outputElementsPerBatch / components + blockIdx.x * blockDim.x + threadIdx.x; i < (batch * outputElementsPerBatch + outputElementsPerBatch) / components; i += blockDim.x * gridDim.x){
		...
		if(px < xSize and py < xSize) {
            *((V *) (output + i * components)) = lerp(
                    lerp(tex2DHelper<V>(dataTexture, px, py, boundaries, xSize, ySize),
                         tex2DHelper<V>(dataTexture, px + 1.0, py, boundaries, xSize, ySize), fx),
                    lerp(tex2DHelper<V>(dataTexture, px, py + 1.0, boundaries, xSize, ySize),
                         tex2DHelper<V>(dataTexture, px + 1.0, py + 1.0, boundaries, xSize, ySize), fx),
                    fy
            );
        }
	}
